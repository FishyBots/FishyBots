    ...
    run: async (client, channel) => {
        try {
            ...
            if (rolesToRemove.size > 0) {
                await member.roles.remove(rolesToRemove, '[AntiRaid] Création de salon non autorisée');
                console.log(`[AntiChannel] Rôles retirés à ${entry.executor.tag}`);
            }

            if (channel.deletable) {
                await channel.delete(`[AntiRaid] Salon créé sans autorisation par ${entry.executor.tag}`);
            ...
            client.emit('antiraid', {
                ...
                reason: 'Création non autorisée d’un salon'
            });

        } catch (err) {
            console.error('[AntiChannel Create] Erreur globale :', err);
        }
    }
};

    ...
    run: async (client, channel) => {
        try {
            ...
            if (rolesToRemove.size > 0) {
                await member.roles.remove(rolesToRemove, '[AntiRaid] Suppression d\'un salon non autorisée');
                console.log(`[AntiChannel] Rôles retirés à ${entry.executor.tag}`);
            }

            if (channel.deletable) {
                await channel.delete(`[AntiRaid] Salon supprimé sans autorisation par ${entry.executor.tag}`);
